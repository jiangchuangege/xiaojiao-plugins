...
function checkDescription(desc, context, errors) {
    ...
    if (!hasMermaid(/\bflowchart\b/))
        errors.push(`${context}: description 缺少数据流转图（mermaid flowchart）`);
    if (!hasMermaid(/\bclassDiagram\b|\bblock-beta\b/))
        errors.push(`${context}: description 缺少接口/模块分解图（mermaid classDiagram / block-beta）`);
    if (!hasMermaid(/\bsequenceDiagram\b/))
        errors.push(`${context}: description 缺少交互时序图（mermaid sequenceDiagram）`);
    const hasCall = fences.some((f) => /\bdigraph\b/.test(f.body) || /graph\s+(TD|LR|RL|BT)\b/.test(f.body));
    if (!hasCall)
        errors.push(`${context}: description 缺少调用图（digraph 或 mermaid graph TD/LR/RL/BT）`);
}
export function parseAtomDocument(text) {
    const errors = [];
    const warnings = [];
    ...
    for (const line of mHead[1].split('\n')) {
        ...
        if (!mm) {
            warnings.push(`frontmatter 行无法解析：${JSON.stringify(line)}`);
            continue;
        }
        const key = mm[1];
        const val = mm[2].trim();
        ...
        if (val === '') {
            ...
            try {
                ...
                errors.push(`frontmatter 键 ${key} 的数组/对象须为合法 JSON 内联值`);
                continue;
            }
        }
        else if (/^"(.*)"$/.test(val)) {
...
export function validateAtomDocumentText(text) {
    ...
    if (parsed.body.trim().length === 0)
        errors.push('atom document: frontmatter 之后缺少 Markdown 正文（四节+四图）');
...
export function validateAtomText(text) {
...
export function validateManifestObject(m, context = 'manifest') {
    ...
    for (const ioKey of ['input', 'output']) {
        ...
        if (!isObj(io) || Object.keys(io).length === 0) {
            ...
            if (!hasShape)
                warnings.push(`${where(ioKey)}: 看起来既无 type 也无结构字段，可能是过弱的数据声明`);
        }
    }
